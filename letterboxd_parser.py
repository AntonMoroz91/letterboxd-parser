...
def collect_user_ratings(user_login):
    ...
    try:
        while True:
            ...
            print(f"DEBUG: Найдено {len(rows)} строк на странице {page_num}")
            ...
            for row in rows:
                try:
                    film_name = None
                    name_elem = row.find_elements(By.CSS_SELECTOR, ".primaryname a")
                    if name_elem:
                        film_name = name_elem[0].text.strip()

                    release_date = None
                    year_elem = row.find_elements(By.CSS_SELECTOR, ".releasedate a")
                    if year_elem:
                        release_date = year_elem[0].text.strip()
                    else:
                        year_elem = row.find_elements(By.CSS_SELECTOR, "td.col-releaseyear span")
                        if year_elem:
                            release_date = year_elem[0].text.strip()
